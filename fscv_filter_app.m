...
% - Save individual filtered I-T traces
    ...
    app.root_directory = pwd;  % Start with current directory
    
    % Create UI controls at top
    ...
    app.txt_status = uicontrol('Style', 'text', 'String', 'Set root directory or load data to begin...', ...
              'Position', [20, 815, 600, 25], ...
              'HorizontalAlignment', 'left', ...
              'ForegroundColor', [0 0 0.8]);
    
    % Create axes for plots
    % Left column: Original data
    app.ax_original = axes('Position', [0.05, 0.4, 0.22, 0.42]);
    title('Original Data');
    
    % Middle column: Filtered data
    app.ax_filtered = axes('Position', [0.32, 0.4, 0.22, 0.42]);
    title('Filtered Data (Click to select)');
    
    % Right column: I vs T comparison
    app.ax_ivst = axes('Position', [0.60, 0.52, 0.36, 0.30]);
    title('Current vs Time');
    grid on;
    
    % Bottom right: Voltammogram
    app.ax_voltammogram = axes('Position', [0.60, 0.08, 0.36, 0.30]);
    title('Voltammogram');
    grid on;
